...
#   if (missingMethod > 0) {
#     if (missingMethod == 1)
#       missingMultiplier <- 0.25
#     else
#       missingMultiplier <- 0.5
#
...
#     simMatrix[i,] <-
#       as.double(
#         rowSums(genotypes[rep(i, numGenotypes),] == genotypes, na.rm = TRUE) +
...
#     stop("allelematch:  missingMethod=0 is currently not implemented",
#          call. = FALSE)
#   }
# }
...
amSimilarityScore <- function(focalGenotypes, comparisonGenotypes=focalGenotypes, minComparableLoci=NULL, missingMethod=NULL) {
  ...
  # ## Determine allele similarity score, fastest version
  # for (i in 1:numFocalGenotypes) {
  #   ## Treat missing data in either the focal or comparison genotype as a partial match except:
  #   ##      When missingMethod=2 missing data matches perfectly with missing data
  #   ##      When missingMethod=1 missing data matches partially with missing data
  #   sumMatching <- rowSums(focalGenotypes[rep(i, numComparisonGenotypes),]==comparisonGenotypes, na.rm=TRUE)
  #   sumMissing  <- rowSums(is.na(comparisonGenotypes) * missingMultiplier) + sum(is.na(focalGenotypes[i,]) * missingMultiplier)
  #   simMatrix[i,] <- as.double(sumMatching + sumMissing)
  # }
  
  # ## Determine allele similarity score, 2 times slower version
  # simMatrix[i,] <- as.double(
  #   rowSums(focalGenotypes[rep(i, numComparisonGenotypes),]==comparisonGenotypes, na.rm=TRUE) +
  #     rowSums(is.na(comparisonGenotypes) * missingMultiplier) + sum(is.na(focalGenotypes[i,]) * missingMultiplier)
  # )
  
  # ## Determine allele similarity score, 5 times slower version
  # for (i in 1:numFocalGenotypes) {
  #   ## Treat missing data in either the focal or comparison genotype as a partial match except:
  #   ##      When missingMethod=2 missing data matches perfectly with missing data
  #   ##      When missingMethod=1 missing data matches partially with missing data
  #   # tsrRowI <- genotypes[i,]
  #   for (j in 1:numComparisonGenotypes) {
  #     sumMatching    <- sum(focalGenotypes[i,]==comparisonGenotypes[j,], na.rm=TRUE)
  #     sumMissing     <- sum(is.na(focalGenotypes[i,]) * missingMultiplier) + sum(is.na(comparisonGenotypes[j,]) * missingMultiplier)
  #     simMatrix[i,j] <- as.double(sumMatching + sumMissing)
  #   }
  # }
